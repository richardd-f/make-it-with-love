...
export default async function CartPage() {
  ...
  if (items.length === 0) {
    return (
      <main className="min-h-screen flex flex-col items-center justify-center gap-6 px-4">
        <div className="flex flex-col items-center justify-center p-12 text-center bg-[#f6e5c4]/10 rounded-3xl border-2 border-dashed border-[#f6e5c4] max-w-md w-full animate-fade-in">
  ...
  return (
    <main className="min-h-screen px-4 py-12 max-w-6xl mx-auto">
      <h1 className="text-4xl font-bold font-family-papernotes text-gray-800 mb-8 animate-fade-in">
        Your Cart ({items.length} {items.length === 1 ? "course" : "courses"})
      </h1>

      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8 items-start animate-fade-in delay-200">
